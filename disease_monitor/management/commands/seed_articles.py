...
class Command(BaseCommand):
    help = 'Seed the database with article tags and sample articles'

    def add_arguments(self, parser):
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing articles and tags before seeding',
        )

    def handle(self, *args, **options):
        if options['clear']:
            self.stdout.write('🗑️ Clearing existing articles and tags...')
        ...
        try:
            ...
            malaria_articles = self.create_malaria_articles(tags)

            self.stdout.write('\n📰 Creating diabetes articles...')
            diabetes_articles = self.create_diabetes_articles(tags)

            self.stdout.write('\n📰 Creating research articles...')
            research_articles = self.create_research_articles(tags)

            # Create research papers
            self.stdout.write('\n🔬 Creating research paper entries...')
            all_articles = malaria_articles + diabetes_articles + research_articles
            research_papers = self.create_research_papers(all_articles)

            # Summary
            self.stdout.write('\n' + '=' * 50)
            self.stdout.write(self.style.SUCCESS('✅ Database seeding completed successfully!'))
            self.stdout.write(f'📊 Created {len(tags)} tags')
            self.stdout.write(f'📰 Created {len(all_articles)} articles')
            self.stdout.write(f'🔬 Created {len(research_papers)} research paper entries')
    ...
    def create_malaria_articles(self, tags):
...
- Findings can guide national malaria programs in resource allocation
        ...
        for article_data in malaria_articles:
            ...
            self.stdout.write(f'✓ Created malaria article: {article.title}')

        return created_articles

    def create_diabetes_articles(self, tags):
...
- Need for improved rural healthcare infrastructure
        ...
        for article_data in diabetes_articles:
            ...
            article.tags.set(tags)
            created_articles.append(article)
            self.stdout.write(f'✓ Created diabetes article: {article.title}')

        return created_articles

    def create_research_articles(self, tags):
...
- Supports evidence-based policy making
        ...
        for article_data in research_articles:
            ...
            article.tags.set(tags)
            created_articles.append(article)
            self.stdout.write(f'✓ Created research article: {article.title}')

        return created_articles

    def create_research_papers(self, articles):
        ...
        for article in articles:
            if article.article_type == 'research':
                ...
                self.stdout.write(f'✓ Created research paper for: {article.title}')
