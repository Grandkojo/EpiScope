      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            Real-time mapping of health risk zones in Ghana
          </p>
        </div>
        <select
          value={filterSeverity}
          onChange={(e) => setFilterSeverity(e.target.value)}
          className="px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-health-500 transition"
        >
          <option value="all">All Severities</option>
          <option value="high">High Risk</option>
          <option value="medium">Medium Risk</option>
        ...
        <CardContent>
          <div className="relative h-96 bg-gradient-to-br from-slate-900/20 to-green-800/10 rounded-xl border border-border overflow-hidden shadow-inner">
            ...
            {filteredHotspots.map((hotspot) => (
              <div
                key={hotspot.id}
                className={`absolute w-4 h-4 rounded-full cursor-pointer transition-all duration-200 hover:scale-150 z-10 ${
                  hotspot.severity === "high"
                    ? "bg-red-500"
                    : hotspot.severity === "medium"
                    ? "bg-yellow-400"
